...
exports.getAllSessions = catchAsync(async (req, res, next) => {
  try {
    ...
    const data = await adminSessionsService.getAllSessions({
      page,
      limit,
      search,
      tagId,
...
exports.getSessionById = catchAsync(async (req, res, next) => {
  try {
    ...
    const sessionData = await adminSessionsService.getSessionByIdWithExercises(
      sessionId
    );
    ...
    res.status(200).json({
      status: "success",
      data: {
        session: sessionData, // Envelopper correctement les données de session
      },
...
exports.createSession = catchAsync(async (req, res, next) => {
  ...
  const session = await adminSessionsService.createSession(userId, {
    name,
    tagIds,
    exercises,
  });
...
exports.updateSession = catchAsync(async (req, res, next) => {
  ...
  const session = await adminSessionsService.updateSession(sessionId, {
    name,
    tagIds,
    exercises,
  });
