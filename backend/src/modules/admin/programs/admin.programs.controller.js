...
exports.getAllPrograms = catchAsync(async (req, res, next) => {
  try {
    ...
    const data = await adminProgramService.getAllPrograms({ page, limit, search, tagId });
...
exports.createProgram = catchAsync(async (req, res, next) => {
  ...
  if (!name || !image || !duration || !Array.isArray(tagIds) || !Array.isArray(sessions)) {
    throw new AppError("Champs manquants ou invalides", 400, "INVALID_BODY");
  }

  const program = await adminProgramService.createProgram(userId, { name, image, duration, tagIds, sessions });
...
exports.updateProgram = catchAsync(async (req, res, next) => {
  ...
  const program = await adminProgramService.updateProgram(programId, { name, image, duration, tagIds, sessions });
  if (!program) {
    throw new AppError("Programme non trouvé", 404, "PROGRAM_NOT_FOUND");
  }
  res.status(200).json({
