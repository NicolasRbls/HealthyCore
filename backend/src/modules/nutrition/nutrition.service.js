...
const NutritionService = {
  ...
  async getNutritionHistory(userId, { startDate, endDate }) {
   ...
   /**
   * Récupère les aliments recommandés selon le profil utilisateur
   * @param {number} userId - Identifiant de l'utilisateur
   * @returns {Array} Liste d'aliments recommandés
   */
  async getRecommendedFoods(userId) {
  try {
    // 1️⃣ Récupérer le profil utilisateur avec poids et objectif
    const profil = await prisma.users.findUnique({
      where: { id_user: Number(userId) },
      include: {
        evolutions: {
          orderBy: { date: "desc" }, // dernier poids enregistré
          take: 1,
        },
        preferences: true, // inclut l’objectif de poids
      },
    });

    if (!profil) throw new Error("Utilisateur non trouvé");

    // 2️⃣ Extraire les valeurs
    const poidsActuel = profil.evolutions?.[0]?.poids;
    const poidsObjectif = profil.preferences?.[0]?.objectif_poids;

    if (!poidsActuel || !poidsObjectif) {
      throw new Error("Données de poids incomplètes");
    }

    // 3️⃣ Déterminer le type d’objectif
    ...
    console.log(`Utilisateur ${userId} → Objectif: ${objectifType}`);

    // 4️⃣ Construire la requête selon l’objectif
    let query;
    if (objectifType === "perte") query = "faible en calories";
    else if (objectifType === "prise") query = "riche en protéines";
    else query = "aliment équilibré";

    // 5️⃣ Appel API OpenFoodFacts
    const foods = await OpenFoodFactsService.searchProducts(query, 20);

    // 6️⃣ Fallback : si rien trouvé, proposer des aliments locaux
    let recommendedFoods = foods;
    if (!foods || foods.length === 0) {
      const localFoods = await prisma.aliments.findMany({
        take: 10,
        where:
          objectifType === "perte"
            ? { calories: { lt: 100 } }
      ...
      recommendedFoods = localFoods.map((f) => ({
        id: f.id_aliment,
        name: f.nom,
        image: f.image,
        type: f.type,
    ...
    console.error("❌ Erreur lors de la récupération des aliments recommandés :", error.message);
    throw error;
  }
}
