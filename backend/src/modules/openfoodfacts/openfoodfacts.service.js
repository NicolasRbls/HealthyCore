...
/**
   * Recommande des aliments selon le profil et l'objectif utilisateur
   * @param {number} userIdParam - ID de l'utilisateur
   */
  async getRecommendations(userIdParam) {
  try {
    // Transformer userId en entier
    ...
    const user = await prisma.users.findUnique({
      ...
      include: {
        objectifs_utilisateurs: true, // si tu veux récupérer les objectifs
      },
    });
...
// 🔹 Afficher le contenu pour debug
    console.log("Objectifs de l'utilisateur :", objectifs);

    //  Construire la query pour OpenFoodFacts selon objectifs
    let query = "";
    if (objectifs.some(o => o.nom_objectif.toLowerCase().includes("protéine"))) {
      query = "riche en proteine";
    } else if (objectifs.some(o => o.nom_objectif.toLowerCase().includes("calorie"))) {
      query = "faible en calories";
    } else {
      query = "aliment sain";
    }

    //  Appel à OpenFoodFacts
    const foods = await OpenFoodFactsService.search(query, 20);
