...
//--------------------------
    // Fetch recommended foods from API
// Fetch recommended foods from API
const fetchRecommendedFoods = async () => {
  try {
    ...
    const foods = await nutritionService.getRecommendedFoods(user.id);
    ...
    if (foods && Array.isArray(foods)) {
      ...
      console.warn("Format inattendu de la réponse :", foods);
    }
  } catch (error) {
    console.error("Erreur recommandations:", error);
    Alert.alert(
      "Erreur",
      "Impossible de récupérer les aliments recommandés. Réessaie plus tard."
    );
  }
};
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {!isLoading && (
        <>
          ...
          >
            {/* Recommended Section */}
            {renderSection("Recommandées", recommendedFoods)}
