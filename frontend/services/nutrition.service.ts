...
const nutritionService = {
  ...
  /**
 * Récupère les aliments recommandés pour un utilisateur donné
 */
async getRecommendedFoods(userId: number): Promise<FoodProduct[]> {
  try {
    const responses = await apiService.get(`/nutrition/recommendations/${userId}`);
    return responses;
  } catch (error) {
